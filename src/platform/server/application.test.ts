...
import { type DBHandle, DBKey } from "../db/db.ts";
import { type ValkeyClient, ValkeyKey } from "../messaging/valkey.ts";
import {
  type HealthRegistry,
  HealthRegistryKey,
  LoggerKey,
...
import { AppKey } from "./http.ts";

const { serveMock, stopFn } = vi.hoisted(() => {
  const stopFn = vi.fn();
  let pending = 0;
...
function makeContainer(cfg = makeConfig()): {
  ...
  // The app under `AppKey` is what feature `register()` functions mount
  // routers onto; `Application.start()` must serve exactly this instance.
  const stubApp = { fetch: vi.fn() } as unknown as Hono;
  container.registerValue(AppKey, stubApp);
  return { container, stubApp, logger };
}
...
describe("Application", () => {
  it("registers dependencies and starts Bun.serve on start()", async () => {
    ...
    const { container, stubApp } = makeContainer();
    const app = new Application(container);
    await app.start();
    expect(serveMock).toHaveBeenCalledTimes(1);
    // Regression: Bun.serve must be handed the `AppKey` app — the same
    // instance feature `register()` mounts routers onto — not a fresh app.
    expect(serveMock).toHaveBeenCalledWith(expect.objectContaining({ fetch: stubApp.fetch }));
