...
import { createExampleRouter } from "./http.ts";
...
describe("example router (HTTP)", () => {
  ...
  it("POST /items with valid body -> 201 + item response", async () => {
    ...
    vi.mocked(service.create).mockResolvedValueOnce(item);
    ...
    expect(service.create).toHaveBeenCalledWith("hello");
  });
  ...
  it("GET /items/:id with valid uuid -> 200 + item", async () => {
    const item = fixedItem("hello");
    vi.mocked(service.get).mockResolvedValueOnce(item);
  ...
  it("GET /items -> 200 with items array", async () => {
    ...
    vi.mocked(service.list).mockResolvedValueOnce(items);
